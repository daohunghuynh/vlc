 ...
 * $Id: 290d2fd4624073e703eca43db5ea28e0fd574baf $
 *
 ...
 * Our NSView object
 *****************************************************************************/
class VLCOpenGLVideoView : NSOpenGLView {

//#define VLCAssertMainThread() assert([[NSThread currentThread] isMainThread])

    private var vd: UnsafePointer<vout_display_t>
    private var _hasPendingReshape: Bool
 ...
 * Gets called by the Open() method.
 */
func setFrameToBoundsOfView(NSValue *)value {
 ...
 * (Non main thread).
 */
func setVoutDisplay(vout_display_t *)aVd {
    @synchronized(self) {
        vd = aVd;
    }
}
 ...
 * (Non main thread).
 */
func setVoutFlushing(BOOL)flushing {
    if (!flushing)
        return;
    @synchronized(self) {
...
func mouseDown(NSEvent *)o_event {
    @synchronized (self) {
        if (vd) {
...
func otherMouseDown(NSEvent *)o_event {
    @synchronized (self) {
        if (vd)
...
func otherMouseUp(NSEvent *)o_event {
    @synchronized (self) {
        if (vd)
...
func mouseMoved(NSEvent *)o_event {
    /* on HiDPI displays, the point bounds don't equal the actual pixel based bounds */
...
func mouseDragged(NSEvent *)o_event {
    [self mouseMoved: o_event];
    [super mouseDragged: o_event];
}

func otherMouseDragged(NSEvent *)o_event {
    [self mouseMoved: o_event];
    [super otherMouseDragged: o_event];
}

func rightMouseDragged(NSEvent *)o_event {
    [self mouseMoved: o_event];
