...
public class VLCVoutWindowController : NSObject {
    ...
    private var voutWindows: [NSValue: VLCVideoWindow] = [:]
    // private VLCKeyboardBacklightControl *keyboardBacklight

    private var topLeftPoint: NSPoint = NSZeroPoint

    // save the status level if at least one video window is on status level
    private var statusLevelWindowCounter: UInt = 0
...
//    }
    ...
    deinit {
        let keys = self.voutWindows.keys
        for key in keys {
            self.removeVout(forWindow: key)
        }
...
//        }
    ...
    func hideMouseForWindow(_ p_wnd: UnsafeMutablePointer<vout_window_t>?) {
        var o_current_window: VLCVideoWindow? = nil

        if p_wnd != nil {
            o_current_window = self.voutWindows[NSValue(pointer: p_wnd!)]
        }
        if o_current_window == nil {
            return
        }
        if NSPointInRect(o_current_window!.mouseLocationOutsideOfEventStream,
                          o_current_window!.videoView.convert(o_current_window!.videoView.bounds, to: nil)) {
            NSCursor.setHiddenUntilMouseMoves(true)
        }
    }

    private var _currentStatusWindowLevel: NSWindow.Level = NSWindow.Level.floating

    var currentStatusWindowLevel: NSWindow.Level {
        return self._currentStatusWindowLevel
    }

    // MARK: - Methods for vout provider

    func setupVoutForWindow(_ p_wnd: UnsafeMutablePointer<vout_window_t>, withProposedVideoViewPosition videoViewPosition: NSRect) -> VLCVoutView {
        let intf = getIntf()!
//        let windowDecorations = var_InheritBool(intf.as_vlc_object_pointer(), "video-deco")
        let multipleVoutWindows = self.voutWindows.count > 0
        ...
        if multipleVoutWindows {
            if self.voutWindows.count == 1 {
                let firstWindow = self.voutWindows[self.voutWindows.keys.first!]!

                let topleftBaseRect = NSMakeRect(0, firstWindow.frame.size.height, 0, 0)
        ...
        self.voutWindows[NSValue(pointer: p_wnd)] = newVideoWindow
    ...
    func removeVout(forWindow: NSValue) {
        if let o_window: VLCVideoWindow = self.voutWindows[forWindow] {
            ...
            self.voutWindows[forWindow] = nil
            if self.voutWindows.count == 0 {
                VLCMain.instance.activeVideoPlayback = false
                self.statusLevelWindowCounter = 0
            }
        } else {
    ...
    func setNativeVideoSize(_ size: NSSize, forWindow: UnsafeMutablePointer<vout_window_t>) {
        if let o_window: VLCVideoWindow = self.voutWindows[NSValue(pointer: forWindow)] {
    ...
    func setWindowLevel(_ i_level: NSWindow.Level, forWindow: UnsafeMutablePointer<vout_window_t>) {

        if let o_window: VLCVideoWindow = self.voutWindows[NSValue(pointer: forWindow)] {
            // only set level for helper windows to normal if no status vout window exist anymore
            if i_level == NSWindow.Level.statusBar {
                self.statusLevelWindowCounter += 1
                ...
                if self.statusLevelWindowCounter > 0 {
                    self.statusLevelWindowCounter -= 1
                }
    ...
    func setFullscreen(_ fullscreen: Bool, forWindow: UnsafeMutablePointer<vout_window_t>?, withAnimation animate: Bool) {
        let p_intf = getIntf()
        ...
        var o_current_window: VLCVideoWindow? = nil
        if forWindow != nil {
            o_current_window = self.voutWindows[NSValue(pointer: forWindow)]
        }
...
//        }
    ...
    func updateControlsBarsUsingBlock(block: (VLCControlsBar) -> Void) {
        for wnd in self.voutWindows.values {
    ...
    func updateWindowLevelForHelperWindows(_ level: NSWindow.Level) {
        ...
        if level == NSWindow.Level.normal {
            self._currentStatusWindowLevel = NSWindow.Level.floating
        } else {
            self._currentStatusWindowLevel = level + 1
        }
...
//        instance.resumeDialog.updateCocoaWindowLevel(statusWindowLevel)
    ...
    public class func WindowOpen(_ p_wnd: UnsafeMutablePointer<vout_window_t>, _ cfg: UnsafePointer<vout_window_cfg_t>) -> Int32 {
        return autoreleasepool { () -> Int32 in
            ...
            if cfg.pointee.type != VOUT_WINDOW_TYPE_INVALID.rawValue
    ...
    @objc public class func WindowControl(_ p_wnd: UnsafeMutablePointer<vout_window_t>?, i_query: UInt32, args: [Int]) -> Int32 {
        ...
        //                     msg_Dbg(p_wnd, "Ignore change to VOUT_WINDOW_STATE_BELOW")
            ...
            case VOUT_WINDOW_SET_SIZE.rawValue:
                ...
                let i_height = args[1]
                DispatchQueue.main.async(execute: {
                    voutController.setNativeVideoSize(NSMakeSize(CGFloat(i_width), CGFloat(i_height)), forWindow: p_wnd!)
                })
    ...
    public class func WindowClose(_ p_wnd: UnsafeMutablePointer<vout_window_t>) {
        autoreleasepool {
            ...
            DispatchQueue.main.async(execute: { [voutController] in
                voutController.removeVout(forWindow: NSValue(pointer: p_wnd))
            })
        }
    }
}
