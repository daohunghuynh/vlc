...
class VLCPlaylist {
  ...
  //   }
    ...
    func createItem(_ itemToCreateDict: [String: Any]) -> UnsafeMutablePointer<input_item_t>? {
        ...
        let optionsArray = itemToCreateDict["ITEM_OPTIONS"] as? [String]

        if FileManager.default.fileExists(atPath: path, isDirectory: &b_dir) && b_dir.boolValue
...
//            msg_Dbg(p_intf, "detected optical media of type %s in the file input", [diskType UTF8String])
  ...
  //   }
    ...
    private func addPlaylistItems(_ array: [Dictionary<String, String>]) {
...
//             i_plItemId = [[[self model] rootItem] plItemId];
        ...
        let b_autoplay = var_InheritBool(getIntf()!.as_vlc_object_pointer(), "macosx-autoplay")
        addPlaylistItems(array, withParentItemId: i_plItemId, atPos: -1, startPlayback: b_autoplay)
    }

    func addPlaylistItems(_ array: [Dictionary<String, String>], tryAsSubtitle isSubtitle: Bool) {
        let p_input = playlist_CurrentInput(pl_Get(getIntf()))

        if isSubtitle && array.count == 1 && p_input != nil {
           let i_result: Int32 = input_AddSlave(p_input, SLAVE_TYPE_SPU, array[0]["ITEM_URL"], true, true, true)
        ...
        addPlaylistItems(array)
    }

    private func addPlaylistItems(_ array: [Dictionary<String, String>],
                                  withParentItemId i_plItemId: Int32,
                                  atPos i_position: Int32,
                                  startPlayback b_start: Bool) {
...
//             PL_UNLOCK;
        ...
        var i_current_offset: Int32 = 0
        
        for i in 0..<array.count {
            var o_current_item = array[i]
            let p_input: UnsafeMutablePointer<input_item_t>? = createItem(o_current_item)
            if p_input == nil {
                continue
            }
            
            let i_pos = (i_position == -1) ? PLAYLIST_END : (i_position + i_current_offset)
            i_current_offset += 1
            let p_item: UnsafeMutablePointer<playlist_item_t>? = playlist_NodeAddInput(p_playlist, p_input, p_parent, i_pos)
            if p_item == nil {
                continue
            }
...
fileprivate func VolumeUpdated(vlc_object_t *p_this, const char *psz_var,
                         vlc_value_t oldval, vlc_value_t new_val, void *param) -> Int32 {
    @autoreleasepool {
